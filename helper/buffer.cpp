...
public:
  ...
  buffer(generator g, binder b, vector<float> &_points,
  ...
  shared_ptr<buffer> set_data(vector<float> &_points,
                              vector<point_type> &_types) {
    points = _points;
    types = _types;
    save_to_gpu();
...
auto DefaultTexture = TexParameters{GL_REPEAT, GL_REPEAT, GL_LINEAR, GL_LINEAR};

class texture : public std::enable_shared_from_this<texture> {
public:
  using pointer = std::shared_ptr<texture>;

  texture(string path, GLenum activeTexture = GL_TEXTURE0,
